...
- **Shared** modules for Config, Types, Remotes, and the Snapshot encoder/decoder. One-script hierarchy per the job posting's preference — no Knit, no Matter, no component systems.
...
- 2 byte header + 8 bytes × 20 enemies = 162 bytes per snapshot
- At 15Hz = ~2.4 KB/s outbound per client at full cap
- Positions quantized to 0.01 stud precision (i16 range ±327 studs, platform is 50×50 so plenty of headroom)
...
Compared to leaving 20 Parts on the server and letting default Roblox replication handle CFrame updates, this is dramatically lighter. Default replication sends full CFrame data (position + quaternion rotation + overhead) at its own rate, and I didn't want to be at the mercy of whatever it decides is "good enough."

## Chase + targeting

Target re-evaluation runs at 4Hz (every 0.25s), not every frame. Players don't teleport, so re-checking the closest player 60 times a second is just CPU I'm burning for no gain. Between re-evals, enemies keep chasing their last known target's live position. This is a meaningful win at higher enemy counts — if I pushed max to 100, target re-eval would still only run 4 times a second, not 100.

The "on platform" check is a 2D bounds test on X/Z (ignores Y since enemies float slightly above). If no player is on the platform, `targetUserId` stays nil and `velocity` stays zero — enemies idle in place. Position clamping prevents enemies from drifting off the platform if chase math overshoots.

## Variation

Variation rolls once per enemy on spawn (color from a preset list, material from a preset list, scale between 0.8 and 1.4). The data is sent in the `EnemySpawned` event and every client constructs the same visual — satisfies the "consistent across all clients" requirement without needing to re-sync it in snapshots.

I tied scale to a speed multiplier (`2 - scale`, so range 0.6×–1.2×) so variation feels mechanical instead of purely cosmetic. Small ones catch up to you faster, big ones lumber. Zero extra replication cost because scale is already in the spawn payload — the speed derivation happens locally on the server.
...
Built client-side as a ScreenGui. Two TextBoxes (spawn rate, cap) and a Kill All button. On FocusLost, the client fires a single `AdminAction` remote with an action name and value. Server validates and clamps (rate 0–10, cap 0–100) before applying. One remote handles all admin actions — easier to extend than one remote per button.
...
- **PathfindingService**: the spec says pathfinding is not required, and on a flat open 50×50 square it'd be overkill. Direct unit-vector chase is the right call.
- **Batching EnemyRemoved for Kill All**: considered sending one remote with a list of IDs instead of N separate remotes. Decided against it — at cap 20 it's instant, and a batched variant adds a code path I'd rather not ship without a reason to.
...
The architecture should hold up past the 20-enemy cap without changes. Snapshot size scales linearly with enemy count (2 + 8N bytes). At 100 enemies that's 802 bytes per snapshot × 15Hz = ~12 KB/s, still well under what a real game's bandwidth budget tolerates. Server-side the main cost is the per-frame iteration of the enemy table and the targeting logic, which are both O(n) and kept thin on purpose.

## Testing process

Developed with Rojo serving a local baseplate. Incremental commits every feature/fix. Validated each phase before moving on — spawner before chase, chase before replication, replication before rendering. Snapshot packet sizes verified at 2 + 8N bytes at each count. Click-to-print verified IDs match between server and client.